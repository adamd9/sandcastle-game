...
describe('POST /mcp', () => {
  ...
  it('submit_move places a block', async () => {
    const res = await request(app)
      .post('/mcp')
      .set('X-Api-Key', 'test-key-p1')
      .send({
        ...
        params: {
          name: 'submit_move',
          arguments: { action: 'PLACE', x: 5, y: 5, block_type: 'packed_sand' },
        },
      });
    expect(res.status).toBe(200);
    const text = res.body.result?.content?.[0]?.text;
    ...
    expect(result.actionsThisTick).toBe(1);
  });

  it('submit_move rejects move in wrong zone', async () => {
    const res = await request(app)
      .post('/mcp')
      .set('X-Api-Key', 'test-key-p1')
      .send({
        ...
        params: {
          name: 'submit_move',
          arguments: { action: 'PLACE', x: 15, y: 5, block_type: 'dry_sand' },
        },
      });
