...
describe('GET /state/history', () => {
  ...
  it('returns all history entries from archive after many ticks', async () => {
    // Fire more ticks than MAX_HISTORY_IN_STORE (10).
    for (let i = 0; i < 12; i++) {
      await request(app).post('/tick').set('X-Api-Key', 'test-key-tick');
    }
    ...
    const res = await request(app).get('/state/history');
    expect(res.status).toBe(200);
    // The history archive stores all entries (not capped like inline state).
